...
class BinaryTree {
    ...
    private fun traversePreOrder(offsetVisit: OffsetVisitor, offset: Offset, node: BinaryNode?, depth: Int, parentOffset: Offset? = null) {
        ...
        if (node?.leftChild != null) traversePreOrder(offsetVisit, Offset(x = offset.x - 1000f - ((node.height + 3).toDouble().pow(5).toFloat()), y = offset.y + 100000f), node?.leftChild, depth + 1, offset)
        if (node?.rightChild != null) traversePreOrder(offsetVisit, Offset(x = offset.x + 1000f + ((node.height + 3).toDouble().pow(5).toFloat()), y = offset.y + 100000f), node?.rightChild, depth + 1, offset)
    }
